...
class InventoryApiController extends Controller
{
  ...
  public function getInventoryByUserIdAndCategory($userId, $categoryId)
  {
    ...
    // البحث عن المنتجات المتعلقة بالفئة المعينة
$products = Item::where('categori_id', $categoryId)
->get();
...
if ($item) {
    ...
    if ($user && $user->price_categoris_id !== null) {
        $priceCategoryItem = PriceCategoryItem::where('price_categoris_id', $user->price_categoris_id)
    ...
    $dataItem = [
        ...
        'percent_sud' => $inventoryItem->percent_sud ?? 0,
        'percent_tl' => $inventoryItem->percent_tl ?? 0,
