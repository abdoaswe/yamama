...
class BranshController extends Controller implements FromCollection, WithHeadings
{
  ...
  public function show($id)
  {
    ...
    // جلب بيانات المستخدم
    $data = User::select('*')->where(['id' => $id, 'role' => 2])->firstOrFail();
    ...
    // المصاريف
    $outlays = Outlay::select('*')
        ->where('user_id', $id)
        ->where('active', '!=', 4)
        ->orderBy('id', 'DESC')->get();

    $outlays_total_tl = $outlays->where('currency', 2)->sum('total');
    $outlays_total_usd = $outlays->where('currency', 1)->sum('total');
    ...
    $total_sales_tl = $dataofsalesOnDay->sum('total_Lera');
    $total_sales_usd = $dataofsalesOnDay->sum('total_Doler');
    ...
    // المخزن
    $totalPriceTL = $inventory->sum(function($item) {
        return $item->price_tl * $item->count;
    });

    $totalPriceUSD = $inventory->sum(function($item) {
        return $item->price_usd * $item->count;
    });
